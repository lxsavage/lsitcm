...
// Determines if the player is `iTunes` (<10.15) or `Music` (>=10.15)
...
module.exports = {
  ...
  getMetadata: async() => new Promise((resolve, reject) => {
    ...
    OSA.executeFile(filename, (err, meta) => {
      ...
      else reject('No song is currently playing')
    })
  }),

  ///TODO: Complete implementation, display
  getPlaylistMetadata: async(name) => new Promise((resolve, reject) => {
    let filename = `${__dirname}/applescript/metadata.playlist.${ itom === 'Music'
      ? 'music.'
      : ''
    }applescript`
    OSA.executeFile(filename, {argv: [name]}, (error, meta) => {
