 ...
 * @copyright Copyright (c) 2018 Logan Savage (r2d2292). This work is licensed under a Creative Commons Attribution-ShareAlike 4.0 International License.
 */
...
module.exports = {
  ...
  playSong: (optObj) => {
    ...
    else if (optobj.name !== undefined || optObj.artist !== undefined) {
      try {
        AAPL.execString(`tell application "iTunes" to play (first track whose name ${(optObj.nameStrict) ? 'is' : 'contains'} "${optObj.name}" and artist is "${optObj.artist}")`)
      }
      catch(e) {
        throw e
      }
    }
    else if (optobj.name !== undefined) {
      try {
        AAPL.execString(`tell application "iTunes" to play (first track whose name ${(optObj.nameStrict) ? 'is' : 'contains'} "${optObj.name}")`)
      }
      catch(e) {
        throw e
      }
    }
    else if (optObj.artist !== undefined) {
      try {
        AAPL.execString(`tell application "iTunes" to play (first track whose artist is ${optObj.artist})`)
      }
      catch(e) {
        throw e
      }
    }
    else if (optobj.album !== undefined) {
      try {
        AAPL.execString(`tell application "iTunes" to play (first track whose album is ${optObj.album})`)
      }
      catch(e) {
        throw e
      }
